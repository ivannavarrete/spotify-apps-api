...
function _updateFavoriteButton(button) {
	...
	button.textContent = isFavorite ? _("sProfileRemoveFavorite") : _("sProfileAddFavorite");
}

function _toggleFavoriteState(user) {
	var uri = user.uri;
	if (favorites.all().indexOf(uri) != -1)
...
function _isFriend(person) {
	...
	return 0 === match.length ? false : true;
}
 ...
 * @param {user} the current user viewing
 * @param {isSelf} is it sp.core.user
 */
function init(user, isSelf) {
	...
	var createButton    = function() {
		if (!isSelf && _isFriend(user) && !added) {
			...
			favoriteButton.textContent = isFavorite ? _("sProfileRemoveFavorite") : _("sProfileAddFavorite");
			...
			r.fromDOMEvent(favorites, "change").subscribe(function(e) {
	...
	tl.fetchList({
		loadTemplateCb: function(obj) {
			...
			var replacements = [
				...
				( "artists" === obj.listtype ? _("sProfileTopArtists") : lang.format(_("sProfileSendTopTracks"), user.name.decodeForText()) )
			];
			...
			if ("artists" !== obj.listtype || isFacebookProfile) {
				...
				dom.query(".sp-button", tl.toplist).forEach(function(elem, index, source) {
		...
		adoptToplistCb: function() {
			if ("tracks" === tl.listtype && !isFacebookProfile) {
	...
	r.fromDOMEvent(form, "submit").subscribe(function(e) {
		...
		var showSendingTracksOverlay = function() {
			...
			innerDiv.innerHTML = lang.format('<img src="sp://import/img/sent-icon.png" /><span>{0}</span>', _("sProfileMusicSent"));
...
function initialize(data, isSelf) {
	...
	if (data) {
		...
		body.innerHTML = lang.format(template.textContent, data.uri, data.name.decodeForText(), " ",
			lang.format(_("sProfileSendTopTracks"), data.name.decodeForText()),
